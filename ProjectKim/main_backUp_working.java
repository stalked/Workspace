...
public class main extends Applet implements Runnable, KeyListener {
	...
	private Image image, grid, playerLoc, chest;
	...
	public void run() {
		...
		while (true) {
			...
			int[][] mapTreasure = new int[5][5];
			
			for (int i=0; i<mapTreasure.length;i++){
				for (int j=0; j<mapTreasure[i].length;j++){
					mapTreasure[i][j] = 0;
				}
			}
			
			for (int i=1; i<mapTreasure.length-1;i++){
				for (int j=1; j<mapTreasure[i].length-1;j++){
					mapTreasure[i][j] = client.getTreasureList2D()[i-1][j-1];
				}
			}
			...
			for (int i=0; i<mapPiece.length; i++){
				for (int j=0; j<mapPiece[i].length; j++){
					if (i!=0 && i!=4 && j!=0 && j!=4){
						mapPiece[i][j].update(mapTreasure[i][j]);
					}
				}
			}
	...
	public void paint(Graphics g) {
		...
		currentX = client.getPlayerX(1);
		currentY = client.getPlayerY(1);
